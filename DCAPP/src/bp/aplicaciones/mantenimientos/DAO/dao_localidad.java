...
public class dao_localidad {

	public Long obtenerNuevoId() throws SQLException, ClassNotFoundException, FileNotFoundException, IOException {
		conexion conexion = new conexion();
		Long id = (long) 0;
		try {
			PreparedStatement consulta = conexion.abrir().prepareStatement("{CALL localidad_obtenerNuevoID()}");
			ResultSet resultado = consulta.executeQuery();
			while (resultado.next()) {
				id = resultado.getLong("id_localidad") + 1;
			}
			resultado.close();
			consulta.close();
	...
	public List<modelo_localidad> obtenerLocalidades(String criterio, int tipo, long id, int limite)
			throws SQLException, ClassNotFoundException, FileNotFoundException, IOException {
		conexion conexion = new conexion();
		List<modelo_localidad> lista_localidades = new ArrayList<modelo_localidad>();
		PreparedStatement consulta = null;
		try {
			consulta = conexion.abrir().prepareStatement("{CALL localidad_obtenerLocalidades(?, ?, ?, ?)}");
			...
			while (resultado.next()) {
				lista_localidades.add(
						new modelo_localidad(resultado.getLong("id_localidad"), resultado.getString("nom_localidad"),
								resultado.getString("des_localidad"), resultado.getString("est_localidad"),
	...
	public void insertarLocalidad(modelo_localidad localidad) throws SQLException,
		...
		try {
			PreparedStatement consulta = null;
			consulta = conexion.abrir().prepareStatement("{CALL localidad_insertarLocalidad(?,?,?,?,?,?)}");
			consulta.setLong(1, id);
			consulta.setString(2, localidad.getNom_localidad().toUpperCase());
			consulta.setString(3, localidad.getDes_localidad().toUpperCase());
			consulta.setString(4, localidad.getEst_localidad());
			consulta.setString(5, localidad.getUsu_ingresa());
			consulta.setTimestamp(6, localidad.getFec_ingresa());
	...
	public void modificarLocalidad(modelo_localidad localidad, modelo_solicitud solicitud, int tipo)
		...
		try {
			PreparedStatement consulta = null;
			consulta = conexion.abrir().prepareStatement("{CALL localidad_modificarLocalidad(?,?,?,?,?,?,?)}");
			consulta.setString(1, localidad.getNom_localidad().toUpperCase());
			consulta.setString(2, localidad.getDes_localidad().toUpperCase());
			consulta.setString(3, localidad.getEst_localidad());
			consulta.setString(4, localidad.getUsu_modifica());
			consulta.setTimestamp(5, localidad.getFec_modifica());
			consulta.setLong(6, localidad.getId_localidad());
	...
	public void actualizarEstadoLocalidad(modelo_localidad localidad) throws SQLException,
			MySQLIntegrityConstraintViolationException, ClassNotFoundException, FileNotFoundException, IOException {
		conexion conexion = new conexion();
		try {
			PreparedStatement consulta = null;
			consulta = conexion.abrir().prepareStatement("{CALL localidad_actualizarEstadoLocalidad(?,?,?,?)}");
			consulta.setString(1, localidad.getEst_localidad());
			consulta.setString(2, localidad.getUsu_modifica());
			consulta.setTimestamp(3, localidad.getFec_modifica());
			consulta.setLong(4, localidad.getId_localidad());
			consulta.executeUpdate();
			consulta.close();
		} catch (SQLException ex) {
			throw new SQLException(ex);
	...
	public void eliminarLocalidad(long id_localidad, modelo_solicitud solicitud, int tipo) throws SQLException,
			MySQLIntegrityConstraintViolationException, ClassNotFoundException, FileNotFoundException, IOException {
		conexion conexion = new conexion();
		conexion.abrir().setAutoCommit(false);
		try {
			PreparedStatement consulta = null;
			consulta = conexion.abrir().prepareStatement("{CALL localidad_eliminarLocalidad(?)}");
			consulta.setLong(1, id_localidad);
			if (tipo == 1) {
				consulta.executeUpdate();
			}
			if (solicitud.getId_mantenimiento() == 3) {
