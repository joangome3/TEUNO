...
import bp.aplicaciones.mantenimientos.modelo.modelo_parametros_generales_3;

public class dao_parametros_generales_3 {

	public List<modelo_parametros_generales_3> obtenerRelacionesOpciones(String campo, String opcion, long localidad,
			int tipo) throws SQLException, ClassNotFoundException, FileNotFoundException, IOException {
		conexion conexion = new conexion();
		PreparedStatement consulta = null;
		List<modelo_parametros_generales_3> lista_relacion = new ArrayList<modelo_parametros_generales_3>();
		try {
			consulta = conexion.abrir().prepareStatement("{CALL parametros_generales_obtenerParametros3(?, ?, ?, ?)}");
			...
			while (resultado.next()) {
				lista_relacion.add(new modelo_parametros_generales_3(resultado.getLong("id_relacion"),
						resultado.getLong("id_empresa"), resultado.getString("nom_empresa"),
						resultado.getLong("id_opcion"), resultado.getLong("id_localidad"),
	...
	public void insertarRelacion(List<modelo_parametros_generales_3> relacionOpcion, long id_opcion, long id_localidad)
		...
		try {
			consulta = conexion.abrir().prepareStatement("{CALL parametros_generales_eliminarParametros3(?, ?)}");
			consulta.setLong(1, id_opcion);
			consulta.setLong(2, id_localidad);
			consulta.executeUpdate();
			for (int i = 0; i < relacionOpcion.size(); i++) {
				Long id = (long) 0;
				consulta = conexion.abrir().prepareStatement("{CALL parametros_generales_obtenerNuevoIDParametros3()}");
				ResultSet resultado = consulta.executeQuery();
				while (resultado.next()) {
					id = resultado.getLong("id_relacion") + 1;
				}
				consulta = conexion.abrir()
						.prepareStatement("{CALL parametros_generales_insertarParametros3 (?, ?, ?, ?, ?, ?, ?)}");
