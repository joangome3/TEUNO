...
public class dao_parametros_generales_11 {

	public List<modelo_parametros_generales_11> obtenerRelacionesOpciones(String opcion, String tipo_servicio,
			String localidad, int tipo)
			throws SQLException, ClassNotFoundException, FileNotFoundException, IOException {
		conexion conexion = new conexion();
		PreparedStatement consulta = null;
		List<modelo_parametros_generales_11> lista_relacion = new ArrayList<modelo_parametros_generales_11>();
		try {
			consulta = conexion.abrir().prepareStatement("{CALL parametros_generales_obtenerParametros11(?, ?, ?, ?)}");
			...
			while (resultado.next()) {
				lista_relacion.add(new modelo_parametros_generales_11(resultado.getLong("id_relacion"),
	...
	public void insertarRelacion(List<modelo_parametros_generales_11> relacionOpcion, long id_opcion, long localidad)
		...
		try {
			consulta = conexion.abrir().prepareStatement("{CALL parametros_generales_eliminarParametros11(?, ?)}");
			...
			for (int i = 0; i < relacionOpcion.size(); i++) {
				...
				consulta = conexion.abrir()
						.prepareStatement("{CALL parametros_generales_obtenerNuevoIDParametros11()}");
				ResultSet resultado = consulta.executeQuery();
				while (resultado.next()) {
					id = resultado.getLong("id_relacion") + 1;
				}
				consulta = conexion.abrir()
						.prepareStatement("{CALL parametros_generales_insertarParametros11 (?, ?, ?, ?, ?, ?, ?)}");
