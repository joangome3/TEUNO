...
public class dao_parametros_generales_8 {

	public List<modelo_parametros_generales_8> obtenerRelacionesEstados(String estado, String color, long localidad,
			int tipo) throws SQLException, ClassNotFoundException, FileNotFoundException, IOException {
		conexion conexion = new conexion();
		PreparedStatement consulta = null;
		List<modelo_parametros_generales_8> lista_relacion = new ArrayList<modelo_parametros_generales_8>();
		try {
			consulta = conexion.abrir().prepareStatement("{CALL parametros_generales_obtenerParametros8(?, ?, ?, ?)}");
			...
			while (resultado.next()) {
				lista_relacion.add(new modelo_parametros_generales_8(resultado.getLong("id_relacion"),
	...
	public void insertarRelacion(List<modelo_parametros_generales_8> relacionOpcion, long id_estado, long id_localidad)
		...
		try {
			consulta = conexion.abrir().prepareStatement("{CALL parametros_generales_eliminarParametros8(?, ?)}");
			consulta.setLong(1, id_estado);
			consulta.setLong(2, id_localidad);
			consulta.executeUpdate();
			for (int i = 0; i < relacionOpcion.size(); i++) {
				Long id = (long) 0;
				consulta = conexion.abrir().prepareStatement("{CALL parametros_generales_obtenerNuevoIDParametros8()}");
				ResultSet resultado = consulta.executeQuery();
				while (resultado.next()) {
					id = resultado.getLong("id_relacion") + 1;
				}
				consulta = conexion.abrir()
						.prepareStatement("{CALL parametros_generales_insertarParametros8 (?, ?, ?, ?, ?, ?, ?)}");
