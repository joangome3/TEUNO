...
public class dao_ubicacion {
	...
	public Long obtenerNuevoId() throws SQLException, ClassNotFoundException, FileNotFoundException, IOException {
		...
		Long id = (long) 0;
		try {
			PreparedStatement consulta = conexion.abrir().prepareStatement("{CALL ubicacion_obtenerNuevoID()}");
	...
	public List<modelo_ubicacion> obtenerUbicaciones(String criterio, String localidad, int tipo, long empresa,
			long tipo_ubicacion, long id, int limite)
			throws SQLException, ClassNotFoundException, FileNotFoundException, IOException {
		conexion conexion = new conexion();
		List<modelo_ubicacion> lista_ubicacions = new ArrayList<modelo_ubicacion>();
		PreparedStatement consulta = null;
		try {
			consulta = conexion.abrir().prepareStatement("{CALL ubicacion_obtenerUbicaciones(?, ?, ?, ?, ?, ?, ?)}");
			...
			ResultSet resultado = consulta.executeQuery();
			while (resultado.next()) {
				lista_ubicacions.add(new modelo_ubicacion(resultado.getLong("id_ubicacion"),
						...
						resultado.getString("pos_ubicacion"), resultado.getLong("id_localidad"),
	...
	public void insertarUbicacion(modelo_ubicacion ubicacion) throws SQLException,
		...
		try {
			PreparedStatement consulta = null;
			consulta = conexion.abrir().prepareStatement("{CALL ubicacion_insertarUbicacion (?, ?, ?, ?, ?, ?, ?, ?)}");
			...
			consulta.setLong(5, ubicacion.getId_localidad());
			consulta.setString(6, ubicacion.getEst_ubicacion());
			consulta.setString(7, ubicacion.getUsu_ingresa());
			consulta.setTimestamp(8, ubicacion.getFec_ingresa());
	...
	public void modificarUbicacion(modelo_ubicacion ubicacion, modelo_solicitud solicitud, int tipo)
		...
		try {
			...
			consulta = conexion.abrir()
					.prepareStatement("{CALL ubicacion_modificarUbicacion (?, ?, ?, ?, ?, ?, ?, ?, ?)}");
			...
			consulta.setLong(4, ubicacion.getId_localidad());
			consulta.setString(5, ubicacion.getEst_ubicacion());
			consulta.setString(6, ubicacion.getUsu_modifica());
			consulta.setTimestamp(7, ubicacion.getFec_modifica());
			consulta.setLong(8, ubicacion.getId_ubicacion());
			consulta.setLong(9, tipo);
			consulta.executeUpdate();
			if (solicitud.getId_mantenimiento() == 2) {
	...
	public void activarDesactivarUbicacion(modelo_ubicacion ubicacion) throws SQLException,
			MySQLIntegrityConstraintViolationException, ClassNotFoundException, FileNotFoundException, IOException {
		conexion conexion = new conexion();
		try {
			PreparedStatement consulta = null;
			consulta = conexion.abrir().prepareStatement("{CALL ubicacion_desactivarUbicacion (?, ?, ?, ?)}");
	...
	public void eliminarUbicacion(Long id_ubicacion) throws SQLException, MySQLIntegrityConstraintViolationException,
		...
		try {
			PreparedStatement consulta = null;
			consulta = conexion.abrir().prepareStatement("{CALL ubicacion_eliminarUbicacion (?)}");
