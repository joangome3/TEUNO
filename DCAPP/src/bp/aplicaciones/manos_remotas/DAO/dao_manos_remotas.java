package bp.aplicaciones.manos_remotas.DAO;
...
public class dao_manos_remotas {

	public Long obtenerNuevoId() throws SQLException, ClassNotFoundException, FileNotFoundException, IOException {
		conexion conexion = new conexion();
		Long id = (long) 0;
		try {
			PreparedStatement consulta = conexion.abrir().prepareStatement("{CALL manos_remotas_obtenerNuevoID()}");
			ResultSet resultado = consulta.executeQuery();
			while (resultado.next()) {
				id = resultado.getLong("id_mano_remota") + 1;
			}
			resultado.close();
			consulta.close();
		...
		try {
			...
			while (resultado.next()) {
				lista_TareasProveedores.add(new modelo_tarea_proveedor(resultado.getLong("id_tarea_proveedor"),
						...
						resultado.getString("id_rack"), resultado.getString("area"),
