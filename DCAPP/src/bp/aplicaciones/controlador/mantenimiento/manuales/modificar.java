...
public class modificar extends SelectorComposer<Component> {
	...
	public void onBlur$txtNombre()
		...
		dao_manual dao = new dao_manual();
		if (dao.obtenerManuales(txtNombre.getText(), String.valueOf(id_dc), 4, manual.getId_manual(), 0).size() > 0) {
	...
	public void onUpload$btnCargar(UploadEvent event) throws SerialException, SQLException, ClassNotFoundException,
		...
		if (media == null) {
			Messagebox.show("No se añadio algún archivo.", ".:: Modificar manual ::.", Messagebox.OK,
		...
		if ((media.getName().indexOf(".pdf") == -1) && (media.getName().indexOf(".xlsx") == -1)
			...
			Messagebox.show("El archivo debe tener extensión (.pdf) o (.xlsx) o (.xls) o (.doc) o (.docx).",
					".:: Modificar manual ::.", Messagebox.OK, Messagebox.INFORMATION);
			return;
		}
		manual.setDir_manual(null);
				...
				Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION, new org.zkoss.zk.ui.event.EventListener() {
					...
					public void onEvent(Event event) throws Exception {
						if (event.getName().equals("onOK")) {
							...
							if (modificar.this.manual.getDir_manual() == null) {
								...
								manual.setDir_manual(modificar.this.manual.getDir_manual());
							}
							manual.setEst_manual("AE");
							manual.setUsu_modifica(user);
							manual.setFec_modifica(fechas.obtenerTimestampDeDate(new Date()));
							try {
								dao.modificarManual(manual, 1);
								Messagebox.show("El manual se guardó correctamente.", ".:: Modificar manual ::.",
								...
								Messagebox.show(
										...
										".:: Modificar manual ::.", Messagebox.OK, Messagebox.EXCLAMATION);
							}
						}
					}
				});
