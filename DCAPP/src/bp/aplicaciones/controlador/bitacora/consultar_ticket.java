...
public class consultar_ticket extends SelectorComposer<Component> {
	...
	public void onDoubleClick$lbxTareasProgramadas() {
		if (lbxTareasProgramadas.getSelectedItem() == null) {
			return;
		}
		modelo_tarea_proveedor tarea_proveedor = null;
		int indice = lbxTareasProgramadas.getSelectedIndex();
		Listitem lItem;
		Listcell lCell;
		lItem = (Listitem) lbxTareasProgramadas.getItemAtIndex(indice);
		lCell = (Listcell) lItem.getChildren().get(0);
		long id_tarea_proveedor = Long.valueOf(lCell.getLabel().toString());
		for (int i = 0; i < listaTareasProgramadas.size(); i++) {
			if (id_tarea_proveedor == listaTareasProgramadas.get(i).getId_tarea_proveedor()) {
				tarea_proveedor = new modelo_tarea_proveedor();
				tarea_proveedor.setId_tarea_proveedor(listaTareasProgramadas.get(i).getId_tarea_proveedor());
				...
				i = listaTareasProgramadas.size() + 1;
			}
		}
		Sessions.getCurrent().setAttribute("tarea_proveedor", tarea_proveedor);
		Events.postEvent(new Event("onClose", zConsultar));
	}
