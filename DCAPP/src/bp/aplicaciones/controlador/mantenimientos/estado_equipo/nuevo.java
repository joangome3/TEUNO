...
public class nuevo extends SelectorComposer<Component> {
	...
	public void onBlur$txtNombre()
		...
		if (dao.consultarEstadosEquipo(0, 0, txtNombre.getText(), "", 0, 3).size() > 0) {
	...
	public void onClick$btnGrabar()
		...
		Messagebox.show("Esta seguro de guardar el estado de equipo?", ".:: Nuevo estado de equipo ::.",
				Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION, new org.zkoss.zk.ui.event.EventListener() {
					...
					public void onEvent(Event event) throws Exception {
						if (event.getName().equals("onOK")) {
							...
							estado_equipo.setUsu_ingresa(user);
							estado_equipo.setFec_ingresa(fechas.obtenerTimestampDeDate(new Date()));
							try {
								dao.insertarEstadoEquipo(estado_equipo);
								Clients.showNotification("El estado de equipo se guardó correctamente.",
										Clients.NOTIFICATION_TYPE_INFO, dSolicitudes, "top_right", 4000);
								limpiarCampos();
							} catch (Exception e) {
								Clients.showNotification(
										"Error al guardar el estado de equipo. \n\n" + "Mensaje de error: \n\n"
												+ e.getMessage(),
