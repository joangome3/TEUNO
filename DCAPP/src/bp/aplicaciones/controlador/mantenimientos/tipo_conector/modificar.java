...
public class modificar extends SelectorComposer<Component> {
	...
	modelo_tipo_conector tipo_conector = (modelo_tipo_conector) Sessions.getCurrent().getAttribute("tipo_conector");
	...
	public void cargarDatos() {
		txtId.setText(String.valueOf(tipo_conector.getId_tipo_conector()));
		txtNombre.setText(tipo_conector.getNom_tipo_conector());
		cmbVoltaje.setText(String.valueOf(tipo_conector.getVol_tipo_conector()) + " V");
	...
	public void onBlur$txtNombre()
		...
		if (dao.consultarTipoConectores(tipo_conector.getId_tipo_conector(), 0, txtNombre.getText(), "", 0, 5)
				.size() > 0) {
	...
	public void onClick$btnGrabar()
		...
		Messagebox.show("Esta seguro de actualizar el tipo de conector?", ".:: Modificar tipo de conector ::.",
				Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION, new org.zkoss.zk.ui.event.EventListener() {
					...
					public void onEvent(Event event) throws Exception {
						if (event.getName().equals("onOK")) {
							...
							tipo_conector = modificar.this.tipo_conector;
							...
							tipo_conector.setUsu_modifica(user);
							tipo_conector.setFec_modifica(fechas.obtenerTimestampDeDate(new Date()));
							try {
								dao.actualizarTipoConector(tipo_conector);
								Clients.showNotification("El tipo de conector se actualizó correctamente.",
										Clients.NOTIFICATION_TYPE_INFO, dSolicitudes, "top_right", 4000, true);
								limpiarCampos();
							} catch (Exception e) {
								Clients.showNotification(
										"Error al actualizar el tipo de conector. \n\n" + "Mensaje de error: \n\n"
												+ e.getMessage(),
