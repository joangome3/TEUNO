...
public class nuevo extends SelectorComposer<Component> {
	...
	public void onBlur$txtNombre()
		...
		if (dao.consultarTipoEquipos(0, 0, txtNombre.getText(), "", 0, 4).size() > 0) {
	...
	public void onClick$btnGrabar()
		...
		Messagebox.show("Esta seguro de guardar el tipo de equipo?", ".:: Nuevo tipo de equipo ::.",
				Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION, new org.zkoss.zk.ui.event.EventListener() {
					...
					public void onEvent(Event event) throws Exception {
						if (event.getName().equals("onOK")) {
							...
							tipo_equipo.setUsu_ingresa(user);
							tipo_equipo.setFec_ingresa(fechas.obtenerTimestampDeDate(new Date()));
							try {
								dao.insertarTipoEquipo(tipo_equipo);
								Clients.showNotification("El tipo de equipo se guardó correctamente.",
										Clients.NOTIFICATION_TYPE_INFO, dSolicitudes, "top_right", 4000, true);
								limpiarCampos();
							} catch (Exception e) {
								Clients.showNotification(
										"Error al guardar el tipo de equipo. \n\n" + "Mensaje de error: \n\n"
												+ e.getMessage(),
