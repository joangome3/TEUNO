...
import bp.aplicaciones.mantenimientos.DAO.dao_parametros_generales_3;
...
public class parametros3 extends SelectorComposer<Component> {
	...
	List<modelo_parametros_generales_3> listaRelacionEmpresa = new ArrayList<modelo_parametros_generales_3>();
	...
	public void onSelect$cmbOpcion()
		...
		dao_parametros_generales_3 dao = new dao_parametros_generales_3();
		long id_empresa = 0;
		long id_opcion = Long.parseLong(cmbOpcion.getSelectedItem().getValue().toString());
		for (int i = 0; i < lbxEmpresas.getItems().size(); i++) {
			id_empresa = listaEmpresa.get(lbxEmpresas.getItemAtIndex(i).getIndex()).getId_empresa();
			...
			if (listaRelacionEmpresa.size() > 0) {
				lbxEmpresas.getItemAtIndex(i).setSelected(true);
			} else {
				lbxEmpresas.getItemAtIndex(i).setSelected(false);
			}
		}
	}
				...
				Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION, new org.zkoss.zk.ui.event.EventListener() {
					...
					public void onEvent(Event event) throws Exception {
						if (event.getName().equals("onOK")) {
							...
							long id_empresa = 0;
							dao_parametros_generales_3 dao = new dao_parametros_generales_3();
							modelo_parametros_generales_3 relacion;
							List<modelo_parametros_generales_3> listaRelacion = new ArrayList<modelo_parametros_generales_3>();
							for (int i = 0; i < lbxEmpresas.getItems().size(); i++) {
								id_empresa = listaEmpresa.get(lbxEmpresas.getItemAtIndex(i).getIndex()).getId_empresa();
								if (lbxEmpresas.getItemAtIndex(i).isSelected()) {
									relacion = new modelo_parametros_generales_3();
